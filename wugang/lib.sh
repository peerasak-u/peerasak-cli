...
elif [ -n "${WUGANG_TIMEOUT_MINUTES:-}" ]; then
  WUGANG_TIMEOUT_SECONDS="$((WUGANG_TIMEOUT_MINUTES * 60))"
else
  WUGANG_TIMEOUT_SECONDS=5400
fi
...
setup_prereqs() {
  ...
  ui_info "Repo: $repo"
  ui_info "Signal timeout: ${WUGANG_TIMEOUT_SECONDS}s"
